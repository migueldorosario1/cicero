#!/usr/bin/env python3
"""Gera CSVs com deputados federais e senadores eleitos do CE em 2022.

Fonte oficial:
https://cdn.tse.jus.br/estatistica/sead/odsele/votacao_candidato_munzona/votacao_candidato_munzona_2022.zip

Saidas padrao:
src/data/deputados_federais_eleitos_ce_2022.csv
src/data/senadores_eleitos_ce_2022.csv
"""
...
CSV_NAME = "votacao_candidato_munzona_2022_CE.csv"
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = SCRIPT_DIR.parent
OUTPUT_DEPUTADOS_FEDERAIS = PROJECT_ROOT / "src" / "data" / "deputados_federais_eleitos_ce_2022.csv"
OUTPUT_SENADORES = PROJECT_ROOT / "src" / "data" / "senadores_eleitos_ce_2022.csv"
JSON_DEPUTADOS_FEDERAIS = PROJECT_ROOT / "src" / "data" / "deputados_federais_eleitos_ce_2022.json"
JSON_SENADORES = PROJECT_ROOT / "src" / "data" / "senadores_eleitos_ce_2022.json"
...
def escrever_json(
    ...
    *,
    ...
    payload = {
        "schemaVersion": "v1",
        "source": "TSE votacao_candidato_munzona_2022_CE.csv",
        "criterio": criterio,
        "total": len(pessoas),
        lista_key: pessoas,
    }
...
def main() -> int:
    ...
    escrever_json(
        ...
        criterio="Deputados federais eleitos em 2022; percentual sobre total de votos nominais para deputado federal no CE.",
    )
    escrever_json(
        senadores,
        args.json_senadores,
        coluna_pessoa="senador",
        lista_key="senadores",
        criterio="Senadores eleitos em 2022; percentual sobre total de votos nominais para senador no CE.",
    )
    ...
    print(f"Total de votos nominais para deputado federal no CE: {total_deputados}")
    print()
    print(f"CSV gerado: {args.out_senadores}")
    print(f"JSON gerado: {args.json_senadores}")
    print(f"Senadores eleitos: {len(senadores)}")
    print(f"Total de votos nominais para senador no CE: {total_senadores}")
